# Preserved from logic/relations.py
# Preservation timestamp: 2025-06-01T23:50:02.444652
# Functions found: 11

#!/usr/bin/env python3
"""
pyics/core/logic/relations.py
Pyics Core Domain Relations: logic
...
PROBLEM SOLVED: Centralized relationship mapping for logic domain
DEPENDENCIES: logic.data_types, typing, dataclasses
...
in the logic domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in logic domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between logic entities
...
class RelationGraph:
    """
    Immutable graph of relations in logic domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, LogicEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[LogicEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for logic relations
...
# Predefined relation mappings for logic domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="LogicEntity",
        target_type="LogicEntity",
...
# [EOF] - End of logic relations.py module
