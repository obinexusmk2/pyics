#!/usr/bin/env python3
"""
pyics/core/transforms/relations.py
Pyics Core Domain Relations: transforms
...
PROBLEM SOLVED: Centralized relationship mapping for transforms domain
DEPENDENCIES: transforms.data_types, typing, dataclasses
...
in the transforms domain, following DOP principles with immutable
...
class RelationType(Enum):
    """Types of relationships in transforms domain"""
    ONE_TO_ONE = auto()
    ONE_TO_MANY = auto()
    MANY_TO_MANY = auto()
    HIERARCHICAL = auto()
...
class Relation:
    """
    Immutable relation between transforms entities
...
class RelationGraph:
    """
    Immutable graph of relations in transforms domain
    
    Container for complete relationship structure
    """
    relations: Tuple[Relation, ...] = field(default_factory=tuple)
    entity_index: Dict[str, TransformsEntity] = field(default_factory=dict)
    ...
    def get_related_entities(self, entity_id: str) -> List[TransformsEntity]:
...
class RelationMapping:
    """
    Static mapping configuration for transforms relations
...
# Predefined relation mappings for transforms domain
DEFAULT_RELATION_MAPPINGS: List[RelationMapping] = [
    RelationMapping(
        mapping_name="hierarchical_parent_child",
        source_type="TransformsEntity",
        target_type="TransformsEntity",
...
# [EOF] - End of transforms relations.py module
